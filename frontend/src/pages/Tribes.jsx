      ...
      <div className="org-customer-grid">
        {customers.map((c) => (
          <div key={c.id} className="org-cchip-wrap">
            ...
            >
              <span
                className="org-cchip-dot"
                style={{ background: STAGE_COLOR[c.stage] || "#6b7280" }}
              />
              <span className="org-cchip-name">{c.customerName}</span>
            </button>
            <button
              className="org-cchip-del"
        ...
        {addOpen ? (
          <div className="org-cchip-form">
            <input
              ...
              autoFocus
            />
            <button className="org-cchip-confirm" onClick={handleAdd} title="Ekle">
              <IconCheck size={11} />
            </button>
            <button className="org-cchip-cancel" onClick={() => { setAddOpen(false); setNewName(""); }} title="İptal">
              <IconX size={11} />
            </button>
          </div>
        ) : (
          <button
            className="org-cchip-add-btn"
...
export default function Tribes({ contracts = [], setContracts, onNavigate }) {
  ...
  const deleteTribe = (tribeId) => {
    ...
    persist(tribes.filter((t) => t.id !== tribeId), (settings.teams || []).filter((t) => !removed.includes(t)));
  };
  ...
  const submitAddTeam = (tribeId) => {
    ...
    if (!name) return;
    const newTribes = tribes.map((t) => t.id === tribeId ? { ...t, teams: [...(t.teams || []), name] } : t);
    persist(newTribes, [...new Set([...(settings.teams || []), name])]);
    setAddTeamFor(null); setNewTeamName("");
  };
  const deleteTeam = (tribeId, teamName) => {
    persist(
      tribes.map((t) => t.id === tribeId ? { ...t, teams: (t.teams || []).filter((n) => n !== teamName) } : t),
  ...
  const handleAddCustomer = (teamName, tribeId, customerName) => {
    ...
    const newContract = {
      id:             `ct-${Date.now()}`,
      customerName,
      contractName:   `${customerName} Sözleşmesi`,
      ...
      actions:        [],
      comments:       [],
      history:        [{ date: today, label: "Active" }],
    };
              ...
              <ul>
                {tribes.map((tribe) => {
                  ...
                  return (
                    <li key={tribe.id}>
                      {/* Tribe node */}
                      <div className="org-node org-node--tribe" style={{ "--tc": tribe.color, borderTopColor: tribe.color }}>
                        <button className="org-node-del" onClick={() => deleteTribe(tribe.id)} title="Tribe'ı sil"><IconX /></button>
                      ...
                      <ul>
                        {(tribe.teams || []).map((teamName) => {
                          ...
                          return (
                            <li key={teamName}>
                              <div className="org-node org-node--team" style={{ borderTopColor: tribe.color }}>
                                <button className="org-node-del org-node-del--sm" onClick={() => deleteTeam(tribe.id, teamName)} title="Takımı sil"><IconX size={10} /></button>
                        ...
                        <li className="org-li--add">
                          {addTeamFor === tribe.id ? (
                            <div className="org-node org-node--input">
                              <input className="org-input" value={newTeamName} onChange={(e) => setNewTeamName(e.target.value)}
                            ...
                            <button className="org-node org-node--ghost" onClick={() => { setAddTeamFor(tribe.id); setNewTeamName(""); setAddTribeOpen(false); }}>
                ...
                <li className="org-li--add">
                  {addTribeOpen ? (
                    <div className="org-node org-node--tribe-form">
                      <input className="org-input" value={newTribeName} onChange={(e) => setNewTribeName(e.target.value)}
